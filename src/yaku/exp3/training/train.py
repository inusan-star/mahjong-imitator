...
# 重み計算の部分を関数化して追加
def get_pos_weights(yaku_names: list) -> torch.Tensor:
    """Calculate positive class weights for each yaku based on distribution."""
    dist_df = pd.read_csv(common_config.YAKU_DISTRIBUTION_FILE).set_index("Yaku Name")
    stats_df = pd.read_csv(exp1_config.DATA_STATS_CSV)
    total_train_samples = int(stats_df.iloc[0, 1])

    weights = []
    for name in yaku_names:
        pos_count = int(dist_df.loc[name].iloc[3])  # 訓練データの正例数
        neg_count = total_train_samples - pos_count
        # pos_weight = negative_samples / positive_samples
        weights.append(neg_count / (pos_count + 1e-6))
...
def train_yakus(yaku_names: list, parsed_args: argparse.Namespace, device: torch.device):
    ...
    # 重み付けの計算と損失関数への適用
    pos_weight = get_pos_weights(yaku_names).to(device)
...
def main(parsed_args: argparse.Namespace):
    ...
    logging.info("Starting Multi-task training for %d Yakus (Experiment 3)", len(yaku_names))
