...
def evaluate_models(indices: list, yaku_names: list, loader: DataLoader, device: torch.device) -> dict:
    ...
    for name in yaku_names:
        model_path = (
            MODEL_DIR / common_config.PROJECT_NAME / exp1_config.GROUP_NAME / name / "best_valid_loss_model.pth"
        )
        model = DNN(
    ...
    for inputs, labels_all in progress_bar:
        ...
        for i, model in enumerate(models):
            ...
            yaku_losses[i] += loss.item() * inputs.size(0)

            all_logits[i].extend(outputs.cpu().numpy().flatten())
            all_labels[i].extend(labels.cpu().numpy().flatten())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--yaku_indices", type=str, default=",".join(map(str, range(33))))
